...
trait DBAdapter {
  ...
  //object keyword declares a singleton. Singleton pattern is part of the language.
  //Lazily instanciated. No statics in scala.
  object Param {
  ...
  //c is a function that takes a Conn as a parameter and returns a sequence of Nodes.
...
//Interacts natively with java libraries. Here scala is being invoked by java
//as a servlet and scala is invoking java jdbc libraries
abstract class WebserviceServlet extends HttpServlet {
  //XML literal syntax checked by compiler. If xml is not wellformed the class will not compile.
  //Code can be embedded using {}
	def soapEnvelope(body:Node) = (<soapenv:Envelope
...
class UserDetailsWSServlet extends WebserviceServlet with DBAdapter {
  ...
  //Partially applied functions (currying) using multiple parameter set stntax.
  //Calling mapUser(c) will return a function that takes a ResultSet and returns Seq[Node]
  ...
  def handlePayload(payload:Node):Node = payload match {
    case (<FindUsersRequest>{x}</FindUsersRequest>) => {
      ...
      (<Users xmlns="urn:www.scalaws.org:UserDetails:v1">
			{withConn(c =>{
			  c.select("select * from user_t where firstName like ? or lastName like ?",
					  search,search)(mapUser(c))
			  })
			}
      </Users>)
    }
